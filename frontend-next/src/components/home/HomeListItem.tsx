...
export function HomeListItem({ post, accentColor = 'text-red-500', showRating = false, compact = false, priority = false }: HomeListItemProps) {
    ...
    return (
        <SpotlightCard
            className={`group bg-[var(--bg-card)] border border-white/5 hover:border-white/10 transition-all rounded-xl overflow-hidden mb-6 relative`}
            ...
            <div className="flex flex-col md:flex-row gap-0 md:gap-6 h-full">
                ...
                <div className="flex-1 flex flex-col justify-center py-4 px-4 md:px-0 md:pr-6 relative z-10">
                    ...
                    <div className="flex items-center gap-2 pl-5 text-xs mb-3 font-medium">
                        <Link href="#" className={`${accentColor} font-bold uppercase tracking-wide flex items-center gap-1 hover:underline cursor-default`}>
